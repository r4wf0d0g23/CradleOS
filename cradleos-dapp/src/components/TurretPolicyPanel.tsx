...
import { CRADLEOS_PKG, CRADLEOS_PKG_V10, CLOCK, SUI_TESTNET_RPC, WELL_KNOWN_TRIBES, eventType } from "../constants";
...
function TurretPolicyPanelInner({ vault, registryClaimer }: { vault: TribeVaultState; registryClaimer: string | null }) {
  ...
  const handleSetLevel = useCallback(async (level: number) => {
    ...
    try {
      const tx = buildSetSecurityLevelTransaction(policyId, vault.objectId, level);
  ...
  const handleToggleAggression = useCallback(async () => {
    ...
    setSecBusy(true); setSecErr(null);
    try {
      const tx = buildSetAggressionModeTransaction(policyId, vault.objectId, !secConfig.aggressionMode);
      const signer = new CurrentAccountSigner(dAppKit);
      await signer.signAndExecuteTransaction({ transaction: tx });
      setTimeout(() => refetchSec(), 2500);
    } catch (e) { setSecErr(e instanceof Error ? e.message : String(e)); }
  ...
  return (
    <div className="card">
      ...
      {/* Member: Apply tribe policy to own structures */}
      {!isFounder && account && (
        <MemberDelegationSection vault={vault} account={account} />
      )}
    </div>
  );
}
...
function PlayerRelationsSection({ vault, policyId, isFounder }: {
  ...
  async function execTx(tx: ReturnType<typeof buildSetPlayerRelationTx>) {
...
function MemberDelegationSection({
  ...
  // Per-structure delegation state from localStorage
  const [delegated, setDelegated] = useState<Record<string, boolean>>(() => {
    const state: Record<string, boolean> = {};
    for (const s of defensiveStructures) {
      state[s.objectId] = !!localStorage.getItem(`delegation:${s.objectId}`);
    }
    return state;
  });

  const handleDelegate = async (s: PlayerStructure) => {
    setBusy(s.objectId);
    ...
    try {
      ...
      tx.moveCall({
        ...
        arguments: [
          ...
          tx.pure.address(vault.objectId), // tribe_vault_id
          tx.object(CLOCK),
        ],
      });
      const signer = new CurrentAccountSigner(dAppKit);
      await signer.signAndExecuteTransaction({ transaction: tx });
      localStorage.setItem(`delegation:${s.objectId}`, vault.objectId);
      ...
      setDelegated(prev => ({ ...prev, [s.objectId]: true }));
    } catch (e) {
  ...
  const handleRevoke = async (s: PlayerStructure) => {
    ...
    try {
      ...
      setDelegated(prev => ({ ...prev, [s.objectId]: false }));
    } catch (e) {
      setErrors(prev => ({ ...prev, [s.objectId]: e instanceof Error ? e.message : String(e) }));
    } finally {
      setBusy(null);
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        textTransform: "uppercase", color: "#FF4700", marginBottom: "4px",
      }}>
        Apply Policy to My Structures
      </div>
      <div style={{ fontSize: "11px", color: "rgba(107,107,94,0.7)", marginBottom: "14px" }}>
        Delegate your turrets and gates to follow this tribe defense policy automatically.
      </div>
      ...
      {!isLoading && defensiveStructures.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {defensiveStructures.map(s => {
            const isDelegated = delegated[s.objectId];
            ...
            return (
              <div key={s.objectId} style={{
                ...
                border: `1px solid ${isDelegated ? "rgba(0,255,150,0.2)" : "rgba(255,71,0,0.12)"}`,
                flexWrap: "wrap",
              }}>
...
async function fetchPolicyDelegations(vaultId: string): Promise<DelegationEntry[]> {
  ...
  try {
    ...
    const [created, revoked] = await Promise.all([
      rpc(1, `${CRADLEOS_PKG_V10}::turret_delegation::DelegationCreated`),
      rpc(2, `${CRADLEOS_PKG_V10}::turret_delegation::DelegationRevoked`),
    ]);
